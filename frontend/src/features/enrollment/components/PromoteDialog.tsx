...
import {
    PromoteEnrollmentPayloadSchema,
...
export function PromoteDialog({
    ...
    submitting: boolean;
    error?: string | null;
    onSubmit: (payload: PromoteEnrollmentPayload) => Promise<void>;
}) {
    const initialToYear = academicYears.find((year) => year.id !== enrollment?.academicYear.id)?.id ?? "";
    const [toAcademicYearId, setToAcademicYearId] = useState<string>(initialToYear);
    ...
    const classOptions = useMemo(
        () => classes.filter((item) => item.academicYearId === toAcademicYearId),
    ...
    async function submit() {
        ...
        const payload: PromoteEnrollmentPayload = {
            studentId: enrollment.student.id,
            fromAcademicYearId: enrollment.academicYear.id,
            toAcademicYearId,
            toClassId,
            toSectionId,
            effectiveDate,
        ...
        const parsed = PromoteEnrollmentPayloadSchema.safeParse(payload);
        if (!parsed.success) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Promote Enrollment</DialogTitle>
                </DialogHeader>
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            ...
                            <SelectContent>
                                {academicYears
                                    .filter((year) => year.id !== enrollment?.academicYear.id)
                                    .map((year) => (
                ...
                <DialogFooter>
                    ...
                    <Button onClick={submit} disabled={!canSubmit}>Promote</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
